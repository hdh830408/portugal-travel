...
function openMuseumModal(id) {
  const data = MUSEUM_ROUTE_DATA.find(d => d.id === id);
  if (!data) return;

  const guideModal = document.getElementById('guideModal');
  if (!guideModal) return;

  document.getElementById('guideEmoji').textContent = "🏛️";
  document.getElementById('guideTitle').textContent = data.title;
  document.getElementById('guideSubtitle').innerHTML = data.subtitle || '';

  let contentHtml = `<div class="guide-section">`;
  contentHtml += `  <div class="guide-history">${data.description}</div>`;
  contentHtml += `</div>`;

  document.getElementById('guideContent').innerHTML = contentHtml;
  guideModal.classList.add('open');
}

window.openMuseumModal = openMuseumModal;

function renderBatalhaPins() {
  const wrapper = document.getElementById('batalhaMapWrapper');
  if (!wrapper || wrapper.dataset.rendered === 'true') return;
  wrapper.dataset.rendered = 'true';
...
function openBatalhaModal(id) {
  const data = BATALHA_ROUTE_DATA.find(d => d.id === id);
  if (!data) return;

  const guideModal = document.getElementById('guideModal');
  if (!guideModal) return;

  document.getElementById('guideEmoji').textContent = "🕍";
  document.getElementById('guideTitle').textContent = data.title;
  document.getElementById('guideSubtitle').innerHTML = data.subtitle || '';
...
window.openBatalhaModal = openBatalhaModal;

function renderAlcobacaPins() {
  const wrapper = document.getElementById('alcobacaMapWrapper');
  if (!wrapper || wrapper.dataset.rendered === 'true') return;
  wrapper.dataset.rendered = 'true';
...
function openAlcobacaModal(id) {
  const data = ALCOBACA_ROUTE_DATA.find(d => d.id === id);
  if (!data) return;
